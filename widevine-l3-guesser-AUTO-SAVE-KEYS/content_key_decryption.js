...
WidevineCrypto.decryptContentKey = async function(licenseRequest, licenseResponse)
{
    ...
    for (currentKey of license.key)
    {
        ...
        dump_config_as_file(`${toHexString(keyId)}:${toHexString(decryptedKey)}`);
...
function dump_config_as_file(config){
  var ele = document.createElement('a');
  ele.href = window.URL.createObjectURL(new Blob([config]));
  ele.download = "wvkeys.txt";
  ele.click();
}
